...
begin
  ...
  lemma boolopt_neq_simps:
    "x \<noteq> Some b \<longleftrightarrow> x=None \<or> x=Some (\<not>b)"
    "x \<noteq> None \<longleftrightarrow> x=Some False \<or> x=Some True"
    apply (cases x rule: boolopt_cases) by auto
  ...
  lemma sem_clause_empty_conv: "sem_clause' C Map.empty = (if C={} then Some False else None)"
    unfolding sem_clause'_def
    by auto
  
  lemma sem_clause_empty_ntrue: "sem_clause' C Map.empty \<noteq> Some True"
    by (auto simp: sem_clause_empty_conv)
  ...
  lemma assign_lit_true[simp]: "sem_lit' l A = Some True \<Longrightarrow> assign_lit A l = A"
    by (cases l) auto  
  ...
  lemma is_syn_taut_alt: "is_syn_taut C \<longleftrightarrow> (\<forall>\<sigma>. sem_clause C \<sigma>)"
    apply rule
    subgoal by simp
    subgoal 
      apply (drule spec[where x="\<lambda>v. Pos v \<notin> C"])
      apply (clarsimp simp: is_syn_taut_conv sem_clause_def)
  ...
  lemma is_blocked_empy_taut: "is_blocked Map.empty C = is_syn_taut C"
    by (auto simp: is_blocked_def sem_clause_empty_conv)
...
subsection \<open>RUP and RAT Criteria\<close>
  ...
  lemma aan_idem[simp]: 
    ...
    by (auto simp: fun_eq_iff assign_all_negated_def map_add_def)
  ...
  lemma aan_dbl: 
    ...
    using assms by (force simp: fun_eq_iff assign_all_negated_def map_add_def)
